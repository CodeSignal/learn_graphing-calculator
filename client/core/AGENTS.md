...
1. `state-manager.js`: Central store with dot-path get/set, history (max 50),
   subscribers per path, optional debug logs. Manages runtime state:
   `parameters` (value/min/max/step, derived from expressions), `functions`
   (from config), `graph` (from config).
2. `event-bus.js`: Namespaced pub/sub (`module:action`), once subscriptions,
   history (max 100), debug logging.
